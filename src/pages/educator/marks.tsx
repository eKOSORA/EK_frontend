...
const Marks = () => {
  ...
  const handleSearchStudents = (e: any) => {
    ...
    //console.log(query)
    const searchedStudents = _studentMarks.filter((student: any) =>
  ...
  const calculateAverage = (selectedArray: Array<any>) => {
    const allMarks = selectedArray.map((studentMark: any) =>
      parseInt(studentMark.records[0].mark)
    );
    const _totalMarks = allMarks.reduce(
      (partialSum: number, a: number) => partialSum + a,
      0
    );
    //console.log(_totalMarks);

    const allMaxMarks = selectedArray.map((studentMark: any) =>
      parseInt(studentMark.records[0].max)
    );
    const _totalMaxMarks = allMaxMarks.reduce(
      (partialSum: number, a: number) => partialSum + a,
      0
    );
    ...
    <div className="text-black animate__animated animate__fadeInLeft bg-[#f0f0f0] min-h-screen">
      {editAsMode === "group" ? (
        <div className="absolute w-full flex items-center justify-center h-screen z-[2] top-0 bg-black/70 left-0">
          <div
            className="absolute z-[-3] w-screen h-screen"
            onClick={() => setEditAsMode("")}
          ></div>
          <div className="z-[10]  h-fit py-10 px-[67px] rounded bg-white flex flex-col items-start justify-start">
            ...
            <div className="w-full flex items-center justify-center">
              <div className=" my-8 flex items-center justify-center">
                <button
                  className="h-10 w-12 rounded-l-lg bg-gray-300 flex items-center justify-center text-2xl"
                  onClick={() =>
                    setMarksFormData({
                      ...marksFormData,
                      marksCount: marksFormData.marksCount - 1,
                    })
                  }
                >
                  -
                </button>
                <div className="h-10 w-16 rounded-none bg-white flex items-center justify-center text-2xl">
                  {marksFormData.marksCount}
                </div>
                <button
                  className="h-10 w-12 rounded-r-lg bg-gray-300 flex items-center justify-center text-2xl"
                  onClick={() =>
                    setMarksFormData({
                      ...marksFormData,
                      marksCount: marksFormData.marksCount + 1,
            ...
            <form
              ...
              onSubmit={handleChangeGroupMarks}
            >
              <FormControl className="my-4" fullWidth>
                ...
                >
                  ...
                  <MenuItem value={marksData.course} className="">
                    {marksData.course
                      .toUpperCase()
                      .split("_")
                      .map((word) => `${word} `)}
              ...
              <div className="flex items-center justify-center">
                <input
                  ...
                  onChange={() => {
                    setMarksFormData({
                      ...marksFormData,
                      notifyParents: !marksFormData.notifyParents,
                    });
                  }}
                  type="checkbox"
                  className=""
              ...
              <button
                type="submit"
                className="rounded-lg py-2.5 px-12 bg-ek-blue-75 cursor-pointer text-white"
              >
                Done
              </button>
            </form>
          ...
          <div className="w-full flex items-start  justify-around mt-6 h-fit">
            <div className=" w-[40%] ">
              <div className="pt-5 px-10 pb-10 border-2 rounded-lg border-ek-blue-50 flex flex-col items-start justify-start">
                <h1 className="font-medium text-4xl font-questrial text-ek-blue-75">
                  Choose
                </h1>
                <FormControl className="my-4" fullWidth>
                  ...
                  >
                    ...
                    {classes.map((course) => (
                      <MenuItem key={Math.random()} value={course.value}>
                        {course.name}
                      </MenuItem>
                    ))}
                  </Select>
                ...
                <FormControl className="my-4" fullWidth>
                  ...
                  >
                    ...
                    {courses.map((year) => (
                      <MenuItem key={Math.random()} value={year.value}>
                        {year.name}
                      </MenuItem>
                    ))}
                  </Select>
              ...
              <div className="neumorphism relative my-4  rounded-lg w-full p-10 flex items-center justify-between">
                <span className="text-[#989898] font-questrial">
                  0 students selected
                </span>
                <button
                  className="font-questrial py-3 px-10 text-white bg-ek-blue-75 btn rounded"
                  onClick={selectEditAsMethod}
                >
                  EDIT
                </button>
                <div className="absolute -bottom-16 right-10  bg-white rounded p-5 editModeSelector hidden flex-col items-start justify-start">
                  ...
                  <div className="flex items-center justify-center">
                    <button
                      className="font-questrial py-2 hover:grayscale-[50%] mr-4 px-10 text-white bg-ek-blue-75 btn rounded"
                      onClick={() => {
                        setEditAsMode("individually");
                        setEditMode(!editMode);
                      }}
                    >
                      {editMode ? "CANCEL" : "INDIVIDUAL"}
                    </button>
                    <button
                      className="font-questrial py-2 mx-4 px-10 hover:grayscale-[50%] text-white bg-ek-blue-75 btn rounded"
                      onClick={() => {
                        setEditAsMode("group");
                        document
                          .querySelector(".editModeSelector")
              ...
              {marksData.course && marksData.class ? (
                <div className="flex flex-col px-6 w-full">
                  <div className="flex items-center justify-between w-full py-4">
                    ...
                    <Link href={"/educator/record/new"}>
                      <IoIosAdd
                        className=" p-1 bg-ek-blue-75 rounded-full cursor-pointer text-white hover:rotate-12 "
                        size={35}
                      />
                    </Link>
                  </div>
                    ...
                    <thead>
                      <tr>
                        <th className="flex items-center justify-center  -w-10 py-4   ">
                          <input
                            onChange={selectAllMarks}
                            className="AllStudentsCheckbox border-white checked:bg-white"
                            type="checkbox"
                            name=""
                            id=""
                          />
                        </th>
                        <th className="w-5/12 bg-ek-blue-50 text-white">
                          Name
                        </th>
                        <th
                          id="marksColumn"
                          className="w-5/12 bg-ek-blue-50 text-white"
                        >
                          /30
                        </th>
                      </tr>
                    ...
                    <tbody>
                      {studentMarks.map((studentMark: any, index: number) => {
                        return (
                          <tr
                            key={Math.random()}
                            className="my-1 even:bg-gray-300 py-1 h-8"
                          >
                            <td className="flex items-center justify-center py-1 -w-10 h-8">
                              <input
                                onChange={() => selectedMarks.push(studentMark)}
                    ...
                    <tfoot className="text-white font-questrial bg-ek-blue-75">
                      <tr>
                        ...
                        <td className="text-center text-xl font-semibold">
                          <span>{Math.round(stats.total * 10) / 10}</span>/
                          <span>{stats.maxTotal}</span>
                        </td>
                      </tr>
                      <tr>
                        <td
                        ...
                        <td className="text-center text-xl font-semibold">
                          <span>{Math.round(stats.average * 10) / 10}</span>/
                          <span>{stats.maxAverage}</span>
                        </td>
                      </tr>
                    </tfoot>
                  </table>
                ...
                <div className="w-full flex items-center justify-center">
                  <span className="text-lg text-gray-400 my-24">
                    Nothing Selected
                  </span>
                </div>
              )}
