...
const Timetables = () => {
    ...
    const [editMode, setEditMode] = useState<EditModeObject>({
        state: false,
        index: undefined
    })
        ...
        <div className='animate__animated animate__fadeInLeft bg-[#f0f0f0] min-h-screen'>
            ...
            <div className='w-full flex h-full items-start justify-start'>
                ...
                <div className={`${sideBarActive ? 'w-10/12' : 'w-full'} flex flex-col items-center justify-start pt-[60px] h-fit px-1 sm:p-10`}>
                    ...
                    <div className='w-full md:w-8/12 lg:w-6/12 flex flex-col sm:px-4 py-3 '>
                        <div className='w-full mb-4 sm:w-2/3 md:w-full flex md:flex-row flex-col items-center justify-center'>
                            <TextField id='from' defaultValue={'00:00'} focused={true} onChange={(e) => setHour({ ...hour, from: e.target.value })} className='w-full sm:w-1/2 my-4  md:w-1/2 mx-4' type={'time'} label="From" />
                            <TextField id='to' defaultValue={'00:00'} focused={true} onChange={(e) => setHour({ ...hour, to: e.target.value })} className='w-full sm:w-1/2 my-4  md:w-1/2 mx-4' type={'time'} label="To" />
                            <button className={'px-4 bg-ek-blue-75 text-white py-2 rounded'} onClick={addHour}>ADD</button>
                        </div>
                        <div className='w-full flex flex-col sm:px-4'>
                    ...
                    {
                        viewTableMode ?
                            <div className='nxl:w-7/12 my-6 flex flex-col items-center justify-center nlg:w-8/12 nmd:w-9/12 nsm10:w-10/12 nsm:w-11/12 w-full'>
                                    ...
                                    <tbody>
                                        {
                                            ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'].map((day: String, index: number) => {
                                                return (
                                                    <tr key={index}>
                                                        <td className='w-40 text-white bg-ek-blue-75 px-4'>{day.toUpperCase()}</td>
                                                        {
                                                            timetable[day].map((subject: LessonInTimeTableObject, index: number) => {
                                                                return (
                                                                    <td key={index}>{subject.initial}</td>
                                                                )
                                                            })
                                                        }
                                                    </tr>
